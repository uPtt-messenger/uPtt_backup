...
class Rijndael(object):
    ...
    def encrypt(self, plaintext):
        ...
        # return ''.join(map(chr, result)).encode('utf8').hex()
        return base64.b64encode(''.join(map(chr, result)).encode('utf8')).decode('utf8')

    def decrypt(self, cipher_text):
        cipher_text = base64.b64decode(cipher_text).decode('utf8')
        # print(cipher_text)
        if len(cipher_text) != self.block_size:
            raise ValueError('wrong block length, expected ' + str(self.block_size) + ' got ' + str(len(cipher_text)))
        Kd = self.Kd
        ...
        for i in range(BC):
            t[i] = (ord(cipher_text[i * 4]) << 24 |
                    ord(cipher_text[i * 4 + 1]) << 16 |
                    ord(cipher_text[i * 4 + 2]) << 8 |
                    ord(cipher_text[i * 4 + 3])) ^ Kd[0][i]
        # apply round transforms
        for r in range(1, ROUNDS):
            for i in range(BC):
...
def gen_key():
    return ''.join(random.choices(string.digits + string.ascii_letters + string.punctuation, k=32))
...
if __name__ == '__main__':
    ...
    # while len(test_string) < 32:
    #     test_string += ' '
    #
    # key = ''.join(random.choices(string.digits + string.ascii_letters + string.punctuation, k=32))
    # print(f'key [{key}]')
    # r = Rijndael(key, block_size=32)
    # ciphertext = r.encrypt(test_string)
    # print(f'ciphertext [{ciphertext}]')
    # plaintext = r.decrypt(ciphertext).rstrip()
    # print(plaintext)

    test_string = 'TestString123'

    key = gen_key()
    cipher_text = encrypt(key, test_string)
